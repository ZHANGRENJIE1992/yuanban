...
class Getieltsdetailinfo(views.APIView):
    ...
    def get(self, request):
        ...
        ielts_info = {
            ...
            'upImgArr': [IMAGES_URL + MEDIA_URL + w for w in ielts.wordimageset.split(',')] if ielts.wordimageset else [],
...
class Getgredetailinfo(views.APIView):
    ...
    def get(self, request):
        ...
        ielts_info = {
            ...
            'upImgArr': [IMAGES_URL + MEDIA_URL + w for w in ielts.wordimageset.split(',')] if ielts.wordimageset else [],
            'upImgArr_read': [IMAGES_URL + MEDIA_URL + r for r in ielts.readimageset.split(',')] if ielts.readimageset else [],
            'upImgArr_blank': [IMAGES_URL + MEDIA_URL + l for l in ielts.fill_blank_imageset.split(',')] if ielts.fill_blank_imageset else [],
            'upImgArr_write': [IMAGES_URL + MEDIA_URL + wr for wr in ielts.writeimageset.split(',')] if ielts.writeimageset else [],
            'upImgArr_math': [IMAGES_URL + MEDIA_URL + s for s in ielts.mathimageset.split(',')] if ielts.mathimageset else [],
            'signdate': datetime.date.strftime(ielts.signdate, "%Y-%m-%d"),
            'buqianstatus': ielts.buqianstatus,
            'username': user.name
        }
...
class Getgmatdetailinfo(views.APIView):
    ...
    def get(self, request):
        ...
        ielts_info = {
            ...
            'upImgArr': [IMAGES_URL + MEDIA_URL + w for w in ielts.wordimageset.split(',')] if ielts.wordimageset else [],
            'upImgArr_read': [IMAGES_URL + MEDIA_URL + r for r in ielts.readimageset.split(',')] if ielts.readimageset else [],
            'upImgArr_grammar': [IMAGES_URL + MEDIA_URL + l for l in ielts.grammarimageset.split(',')] if ielts.grammarimageset else [],
            'upImgArr_write': [IMAGES_URL + MEDIA_URL + wr for wr in ielts.writeimageset.split(',')] if ielts.writeimageset else [],
            'upImgArr_math': [IMAGES_URL + MEDIA_URL + s for s in ielts.mathimageset.split(',')] if ielts.mathimageset else [],
            'upImgArr_logic': [IMAGES_URL + MEDIA_URL + s for s in ielts.logicimageset.split(',')] if ielts.logicimageset else [],
            'signdate': datetime.date.strftime(ielts.signdate, "%Y-%m-%d"),
            'buqianstatus': ielts.buqianstatus,
            'username': user.name
        }
